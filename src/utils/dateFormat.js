 ...
 * @LastEditTime: 2020-08-31 15:52:58
 * @LastEditors: 郑晶
 * @Description: 时间转换函数
 */
/**
...
export const dateFormat = (parDate, fmt) => {
	...
	if (/(y+)/.test(fmt)) {
		fmt = fmt.replace(RegExp.$1, (parDate.getFullYear() + '').substr(4 - RegExp.$1.length));
	}
	if (/(E+)/.test(fmt)) {
		fmt = fmt.replace(
			RegExp.$1,
...
export const dateFilter = data => {
	if ((new Date() - new Date(data)) / 3600 / 1000 > 24) {
		...
		if ((new Date() - new Date(data)) / 3600 / 1000 > 1) {
			...
			return parseInt(((new Date() - new Date(data)) / 3600 / 1000) * 60) + '分钟前';
		}
	}
};
